...
class ExchangeratesApiProviderTest {
    ...
    private static final String API_URL = "https://api.exchangeratesapi.io/v1";
    ...
    @BeforeEach
    void setUp() {
        provider = new ExchangeratesApiProvider(restTemplate, API_URL, API_KEY);
    }
    ...
    void fetchLatestRates_ShouldReturnRates_WhenResponseIsSuccessful() {
        // Arrange
        ExchangeratesApiResponse response = new ExchangeratesApiResponse();
        ...
        verify(restTemplate).getForObject(urlCaptor.capture(), eq(ExchangeratesApiResponse.class));
    ...
    void getProviderName_ShouldReturnCorrectName() {
        ...
        // Assert
        assertThat(result).isEqualTo("exchangeratesapi.io");
    }
    ...
    @Test
    @DisplayName("fetchLatestRates should handle special characters in base currency")
    void fetchLatestRates_ShouldHandleSpecialCharactersInBaseCurrency() {
        // Arrange
        ExchangeratesApiResponse response = new ExchangeratesApiResponse();
        response.setSuccess(true);
        response.setRates(new HashMap<>());
        ...
        // Act
        Map<String, BigDecimal> result = provider.fetchLatestRates("USD");

        // Assert
        verify(restTemplate).getForObject(urlCaptor.capture(), eq(ExchangeratesApiResponse.class));
        assertThat(urlCaptor.getValue()).contains("base=USD");
    }

    @Test
    @DisplayName("fetchHistoricalRate should handle leap year date")
    void fetchHistoricalRate_ShouldHandleLeapYearDate() {
        // Arrange
        LocalDate leapDate = LocalDate.of(2024, 2, 29);
        ExchangeratesApiResponse response = new ExchangeratesApiResponse();
        response.setSuccess(true);
        Map<String, BigDecimal> rates = new HashMap<>();
        rates.put(TARGET_CURRENCY, new BigDecimal("0.88"));
        response.setRates(rates);
        ...
        // Act
        BigDecimal result = provider.fetchHistoricalRate(BASE_CURRENCY, TARGET_CURRENCY, leapDate);

        // Assert
        assertThat(result).isEqualByComparingTo("0.88");
        verify(restTemplate).getForObject(urlCaptor.capture(), eq(ExchangeratesApiResponse.class));
        assertThat(urlCaptor.getValue()).contains("/2024-02-29");
    }
}
