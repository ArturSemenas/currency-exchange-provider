...
/**
 * Client for Mock Provider 1 (Fixer.io simulation).
 * Fetches exchange rates from standalone mock service on port 8091.
...
public class MockProvider1Client implements ExchangeRateProvider {
    ...
    private static final String LATEST_ENDPOINT = "/api/v1/latest";
    private static final String HISTORICAL_ENDPOINT = "/api/v1/{date}";
     ...
     * @param baseUrl the base URL of mock provider 1
     */
    public MockProvider1Client(RestTemplate restTemplate,
                               @Value("${api.mock.provider1.url}") String baseUrl) {
    ...
    public Map<String, BigDecimal> fetchLatestRates(String baseCurrency) {
        try {
            ...
            log.debug("Fetching latest rates from Mock Provider 1 for base: {}", baseCurrency);
            FixerIoResponse response = restTemplate.getForObject(url, FixerIoResponse.class);
            
            if (response != null && response.isSuccess()) {
                log.info("Successfully fetched {} rates from Mock Provider 1 for base {}", 
            ...
            log.error("Error fetching latest rates from Mock Provider 1: {}", e.getMessage());
            return new HashMap<>();
        }
    }
    ...
    public BigDecimal fetchHistoricalRate(String baseCurrency, String targetCurrency, LocalDate date) {
        try {
            ...
            log.debug("Fetching historical rate from Mock Provider 1: {} to {} on {}", 
            ...
            if (response != null && response.isSuccess() && response.getRates() != null) {
                ...
                log.info("Successfully fetched historical rate from Mock Provider 1: {} -> {} = {}", 
            ...
            log.error("Error fetching historical rate from Mock Provider 1: {}", e.getMessage());
            return null;
        }
    }
    ...
    public boolean isAvailable() {
        try {
            ...
            if (!available) {
                log.debug("Mock Provider 1 returned empty rates");
            }
            return available;
        } catch (Exception e) {
            log.warn("Mock Provider 1 is not available: {}", e.getMessage());
            return false;
        }
    }
}
